    ...
    def open_images(self):
        ...
        for folder_name in os.listdir(self.folder):
            ...
            for image_name in os.listdir(os.path.join(self.folder, folder_name)):
                image = cv2.imread(os.path.join(self.folder, folder_name, image_name), 0)
                image = cropp_image(image)
...
def recognize():
    ...
    image = cropp_image(np.array(img)).flatten()
    ...
    return jsonify(list(db.people_images.keys())[math.floor((res.index(min(res)) + 1) / 10)])


if __name__ == '__main__':
    app.run(host='192.168.1.47', debug = True)
